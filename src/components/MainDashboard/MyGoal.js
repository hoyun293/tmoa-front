...
import { addComma2Number, getPercent } from '../../js/CommonFunc';
    ...
    <Card>
      <Row>
        ...
        <Col span={18} style={{textAlign:'right', paddingTop:'0.5rem'}}>
          <span style={{fontWeight:500, fontSize:'1.5rem'}}>목표금액 {addComma2Number(target.targetAmount)}원</span>
        </Col>
      </Row>
      <Row>
        <Col span={6} style={{paddingLeft:'0.7rem', fontWeight:600}}>
          <span style={{fontSize: '2.7rem'}}>{getPercent(target.targetAmount, target.currentAmount)}</span>
          <span style={{fontSize: '2rem'}}>%</span>
        </Col>
        <Col span={18} style={{textAlign:'right', paddingTop:'1.1rem'}}>
          <span style={{fontWeight:'bold', fontSize:'1.6rem'}}>{addComma2Number(target.currentAmount)}원</span>
        </Col>
      </Row>
      <Row>
        <Col span={24}>
          <Progress percent={getPercent(target.targetAmount, target.currentAmount)} strokeColor="green" showInfo={false} /> 
        </Col>
      </Row>
      <Row justify="end">
        <Col span={3}>
      ...
      <Row>
        <Col span={24}>
          <p style={{fontWeight:600, fontSize:'1.7rem', margin:0 }}>코로나 끝나고 여행가기</p>
        </Col>
        <Col span={24}>
          <ul style={{display:'flex', listStyle: 'none', margin:0, padding:0 }}>
            {target.tagList.map((tag, index) => {
              return (
                <li key={index} style={{fontColor:'#dddddd'}}>#{tag}</li>
              );
            })}
          </ul>
        </Col>
